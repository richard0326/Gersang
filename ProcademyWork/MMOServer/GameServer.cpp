...
bool CGameServer::Init(CEchoServer* EchoServer)
{
...
bool CGameServer::Start()
{
	...
	if (gameParser.ReadBuffer(L"mmoInfo.txt", &errMsg) == false)
	{
	...
	int COMPLETEQ_SIZE = 0;
	if (gameParser.GetValue(L"Network", L"COMPLETEQ_SIZE", &COMPLETEQ_SIZE) == false)
		return false;

	int SOCKETQ_SIZE = 0;
	if (gameParser.GetValue(L"Network", L"SOCKETQ_SIZE", &SOCKETQ_SIZE) == false)
		return false;

	LOG(L"System", en_LOG_LEVEL::LEVEL_SYSTEM, L"Parser 세팅 완료");

	if (m_EchoServer->Start(LISTEN_IP, LISTEN_Port,
		...
		RINGBUFFER_SIZE, // 링버퍼 크기
		COMPLETEQ_SIZE, // 완료통지 큐 크기
		SOCKETQ_SIZE // 소켓 큐 크기
	) == false)
		return false;
