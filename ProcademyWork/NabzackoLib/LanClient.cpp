...
bool CLanClient::Start(const wchar_t* ipWstr, int portNum,
	...
	bool bNoDelayOpt, int serverMaxUser, bool bRSTOpt, bool bKeepAliveOpt, bool bOverlappedSend, int SendQSize, int RingBufferSize)
{
	...
	if (ConnectSocket == INVALID_SOCKET) {
		LOG(L"LanClientSystem", en_LOG_LEVEL::LEVEL_SYSTEM, L"socket() Error : %d", WSAGetLastError());
		return false;
	}
	LOG(L"LanClientSystem", en_LOG_LEVEL::LEVEL_SYSTEM, L"socket() Success");

	sockaddr_in ConnectInfo;
	ZeroMemory(&ConnectInfo, sizeof(ConnectInfo));
	ConnectInfo.sin_family = AF_INET;
	InetPton(AF_INET, ipWstr, &ConnectInfo.sin_addr);
	ConnectInfo.sin_port = htons(portNum);

	int iResult = ::connect(ConnectSocket, (sockaddr*)&ConnectInfo, sizeof(ConnectInfo));
	if (iResult == SOCKET_ERROR) {
		LOG(L"LanClientSystem", en_LOG_LEVEL::LEVEL_SYSTEM, L"connect() Error : %d", WSAGetLastError());
		return false;
	}
	LOG(L"LanClientSystem", en_LOG_LEVEL::LEVEL_SYSTEM, L"[ IP %s, Port %d ] connect() Success", ipWstr, portNum);

	if (m_bNoDelayOpt)
	{
		BOOL opt = TRUE;
		setsockopt(ConnectSocket, IPPROTO_TCP, TCP_NODELAY, (const char*)&opt, sizeof(opt));

		LOG(L"LanClientSystem", en_LOG_LEVEL::LEVEL_SYSTEM, L"NoDelay Opt True");
	}
	...
	if (m_bRSTOpt)
	{
		...
		optval.l_onoff = 1;
		optval.l_linger = 0;
		setsockopt(ConnectSocket, SOL_SOCKET, SO_LINGER, (char*)&optval, sizeof(optval));

		LOG(L"LanClientSystem", en_LOG_LEVEL::LEVEL_SYSTEM, L"RST Opt True");
	}
	...
	if (m_bKeepAliveOpt)
	{
		...
		int retIOCTL = WSAIoctl(ConnectSocket, _WSAIOW(IOC_VENDOR, 4), &keepAlive, sizeof(tcp_keepalive), 0, 0, &dwBytesReturned, NULL, NULL);
		if (retIOCTL != 0)
		{
			int WSAError = WSAGetLastError();
			if (WSAError != WSA_IO_PENDING)
			{
				LOG(L"LanClientSystem", en_LOG_LEVEL::LEVEL_SYSTEM, L"KeepAliveOpt Fail : %d ", WSAError);
				return false;
			}
		}
		LOG(L"LanClientSystem", en_LOG_LEVEL::LEVEL_SYSTEM, L"KeepAliveOpt On, waitDelay %d, waitInterval %d", keepAlive.keepalivetime, keepAlive.keepaliveinterval);
	}

	m_hIOCP = CreateIoCompletionPort(INVALID_HANDLE_VALUE, 0, 0, workerRunningCnt);
	if (m_hIOCP == INVALID_HANDLE_VALUE)
	{
		LOG(L"LanClientSystem", en_LOG_LEVEL::LEVEL_SYSTEM, L"CreateIoCompletionPort() Error : %d", WSAGetLastError());
		return false;
	}
	LOG(L"LanClientSystem", en_LOG_LEVEL::LEVEL_SYSTEM, L"CreateIoCompletionPort() Success");

	m_Session = new st_SESSION;

	m_Session = CreateSession(ConnectSocket);

	m_Session->SendQ = new CLockFreeQueue<CSerializeBuffer*>(SendQSize);
	m_Session->RecvQ = new CRingBuffer(RingBufferSize);

	LOG(L"LanClientSystem", en_LOG_LEVEL::LEVEL_SYSTEM, L"SendQSize : %d, RingBufferSize : %d",
		SendQSize, RingBufferSize);
	
	m_hIOThread = (HANDLE)_beginthreadex(nullptr, 0, CLanClient::netIOThread, this, 0, nullptr);
	
	LOG(L"LanClientSystem", en_LOG_LEVEL::LEVEL_SYSTEM, L"Server Start...");

	// 송신버퍼 0으로 해서 비동기 방식으로 작동
	if (m_bOverlappedSend == true)
	{
		int sendSize = 0;
		setsockopt(ConnectSocket, SOL_SOCKET, SO_SNDBUF, (char*)&sendSize, sizeof(int));
	}
	CreateIoCompletionPort((HANDLE)ConnectSocket, m_hIOCP, (ULONG_PTR)m_Session, 0);

	DEBUG(DEBUG_LOCATION::ACCEPT_BEF_ONCLIENTJOIN, m_Session);
	OnClientJoin();
	DEBUG(DEBUG_LOCATION::ACCEPT_AFT_ONCLIENTJOIN, m_Session);

	DEBUG(DEBUG_LOCATION::ACCEPT_BEF_RECVPOST, m_Session);

	int retRecvPost = netWSARecvPost();
	if (retRecvPost != RET_SUCCESS)
	{
		DisconnectSession();
	}

	DEBUG(DEBUG_LOCATION::ACCEPT_AFT_RECVPOST, m_Session);
...
void CLanClient::Stop()
{
	...
	LOG(L"LanClientSystem", en_LOG_LEVEL::LEVEL_SYSTEM, L"Server Stopping... Wait 1Min...");

	PostQueuedCompletionStatus(m_hIOCP, 0, 0, 0);

	// 1초 동안 기다린다...
	DWORD waitCount = WaitForSingleObject(m_hIOThread, 60000);
	if (waitCount == WAIT_TIMEOUT)
	{
		TerminateThread(m_hIOThread, 0);
		LOG(L"LanClientSystem", en_LOG_LEVEL::LEVEL_SYSTEM, L"Server Stop. Terminate Worker Thread");
	}
	...
	delete m_Session->SendQ;
	delete m_Session->RecvQ;
	delete m_Session;

	LOG(L"LanClientSystem", en_LOG_LEVEL::LEVEL_SYSTEM, L"Server Stop...");
}

unsigned __stdcall CLanClient::netIOThread(void* pParameter)
{
	CLanClient* pLanClient = (CLanClient*)pParameter;
	...
	while (1)
	{
		...
		GetQueuedCompletionStatus(pLanClient->m_hIOCP, &transferred, (PULONG_PTR)&pSession, (LPOVERLAPPED*)&pOverlapped, INFINITE);
...
#ifdef MY_DEBUG
		pSession->tlsIndex = pLanClient->m_tlsDebugIndex;
		...
		if (transferred == 0)
		{
			pLanClient->ClientDisconnectSession();
			// 세션 종료
		...
		else
		{
			if (pOverlapped->recvFlags == true)
			{
				...
				else
				{
					...
					int retRecv = pLanClient->netWSARecv();
			...
			else
			{
...
#ifdef MY_DEBUG
					pPacket->SetLog(13, (unsigned long long)pSession);
#endif
					pPacket->DecreaseRefCount();
					InterlockedIncrement64(&pLanClient->m_SendTpsCount);
				}
				...
				int retSendPost = pLanClient->netWSASendPost();
		...
		pLanClient->DecreaseIOCount();
	}

	return 0;
}

int CLanClient::netWSARecv()
{
	st_SESSION* pSession = m_Session;
	while (1)
	{
		short Header = 0;
		...
		InterlockedIncrement64(&m_RecvTpsCount);

		DEBUG(DEBUG_LOCATION::RECV_BEF_ONRECV, pSession);

		int retOnRecv = OnRecv(RecvPacket);
...
int CLanClient::netWSARecvPost()
{
	...
	if (afterEnqSize == 0)
	{
		...
		if (retRecv != 0)
		{
			...
			if (WSAError != WSA_IO_PENDING)
			{
				PrintError(L"netWSARecvPost() WSARecv Error ", WSAError);
...
int CLanClient::netWSASendPacket(CSerializeBuffer* pPacket)
{
	st_SESSION* pSession = AcquireLock();
	if (pSession == nullptr)
	{
		return RET_SOCKET_ERROR;
	}
...
#ifdef MY_DEBUG
	pPacket->SetLog(10, (unsigned long long)pSession);
#endif
...
#ifdef MY_DEBUG
		pPacket->SetLog(11, (unsigned long long)pSession);
#endif
		pPacket->DecreaseRefCount();
		ReleaseLock();
		return RET_SOCKET_ERROR;
	...
	int retSendPost = netWSASendPost();
	if (retSendPost != RET_SUCCESS)
	{
		ReleaseLock();
		return retSendPost;
...
int CLanClient::netWSASendEnq(CSerializeBuffer* pPacket)
{
	st_SESSION* pSession = AcquireLock();
	if (pSession == nullptr)
	{
...
int CLanClient::netWSASendPost()
{
	...
	if (retIOSend == 0)
	{
...
#ifdef MY_DEBUG
			pPacket->SetLog(12, (unsigned long long)pSession);
#endif
		}
		...
		if (sendErr != 0)
		{
			...
			if (WSAError != WSA_IO_PENDING)
			{
				PrintError(L"WSASend() Error ", WSAError);
...
st_SESSION* CLanClient::CreateSession(SOCKET clientSocket)
{
	st_SESSION* pSession = m_Session;
...
bool CLanClient::GetIP_Port(wchar_t* outIpStr, unsigned short* outPortNum)
{
	st_SESSION* pSession = AcquireLock();
	if (pSession == nullptr)
		return false;
...
void CLanClient::ClientDisconnectSession()
{
	st_SESSION* pSession = m_Session;
	...
	if (CancelIoEx((HANDLE)pSession->Socket, nullptr) == 0)
	{
		...
		{
			PrintError( L"Disconnect2() Error ", WSAError);
...
void CLanClient::DisconnectSession()
{
	...
	if (CancelIoEx((HANDLE)pSession->Socket, nullptr) == 0)
	{
		...
		if (WSAError != ERROR_NOT_FOUND)
		{
			PrintError(L"Disconnect1() Error ", WSAError);
...
void CLanClient::DecreaseIOCount()
{
	st_SESSION* pSession = m_Session;
#ifdef DEBUG
		...
		if (InterlockedCompareExchange((LONG*)&pSession->releaseFlag, true, false) == false)
		{
...
#ifdef MY_DEBUG
				pPacket->SetLog(14, (unsigned long long)pSession);
#endif
				pPacket->DecreaseRefCount();
			}
			pSession->PacketArrLen = 0;
			...
			for (;;)
			{
				...
				if (SendQ->Dequeue(&pPacket) == false)
					break;
#ifdef MY_DEBUG
				pPacket->SetLog(15, (unsigned long long)pSession);
#endif
				pPacket->DecreaseRefCount();
			}
#ifdef MY_DEBUG
...
void CLanClient::PrintError(const wchar_t* errMsg, int errorCode)
{
	if (!(errorCode == 10004 || errorCode == 10022 || errorCode == 10053 || errorCode == 10054 || errorCode == 10058))
	{
		...
		OnError(m_Session->IpStr, m_Session->usPort, errorCode, errMsg);
	}
}

st_SESSION* CLanClient::AcquireLock()
{
	st_SESSION* pSession = m_Session;
...
void CLanClient::ReleaseLock()
{
	DEBUG(DEBUG_LOCATION::RELEASE_BEGIN, m_Session);
	DecreaseIOCount();
}
