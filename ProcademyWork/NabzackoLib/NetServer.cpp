...
struct st_SESSION
{
	...
	// Packet 사용 후 완료통지에서 참조카운트 내려주기 위한 멤버 
	...
	// 캐시라인 한번 벌려줄 겸
	// 자주 사용하는 Sendflag를 64 캐시라인을 할당해준다.
...
bool CNetServer::Start(const wchar_t* ipWstr, int portNum,
	...
	bool bNoDelayOpt, int serverMaxUser, bool bRSTOpt, bool bKeepAliveOpt, bool bOverlappedSend, int SendQSize, int RingBufferSize)
{
	...
	// 모니터링 정보들 (Worker 스레드 + Accept + 내외부 스레드 여유분)
	...
	{
		m_hIOThread[i] = (HANDLE)_beginthreadex(nullptr, 0, CNetServer::netIOThread, this, 0, nullptr);
	}

	m_hAcceptThread = (HANDLE)_beginthreadex(nullptr, 0, CNetServer::netAcceptThread, this, 0, nullptr);
	m_hMonitoringThread = (HANDLE)_beginthreadex(nullptr, 0, CNetServer::monitoringThread, this, 0, nullptr);
...
unsigned __stdcall CNetServer::monitoringThread(void* pParameter)
{
	CNetServer* pNetServer = (CNetServer*)pParameter;
	srand((unsigned int)GetCurrentThreadId());

	int* pSessionCount = &pNetServer->m_SessionCnt;
	long long* pAcceptCnt = &pNetServer->m_acceptCount;
	long long* pAcceptPrevCnt = &pNetServer->m_acceptPrevCount;

	int* pTlsCount = &pNetServer->m_tlsCount;
	long long* pRecvArr = pNetServer->m_pTlsRecvTpsArr;
	long long* pRecvPrevArr = pNetServer->m_pSaveRecvTpsArr;
	long long* pSendArr = pNetServer->m_pTlsSendTpsArr;
	long long* pSendPrevArr = pNetServer->m_pSaveSendTpsArr;

#ifdef MY_DEBUG
	long long* pDisconnectCnt = &pNetServer->m_DisconnectCnt;
	
	long long* pRecvBPS = pNetServer->m_pRecvBytePerSecArr;
	long long* pRecvPrevBPS = pNetServer->m_pSaveRecvBytePerSecArr;
	long long* pSendBPS = pNetServer->m_pSendBytePerSecArr;
	long long* pSendPrevBPS = pNetServer->m_pSaveSendBytePerSecArr;
#endif

	CLockFreeTlsPoolA<CSerializeBuffer>* pPacketTlsPool = pNetServer->m_packetTlsPool;
	...
	while (true)
	{
		...
		*pAcceptPrevCnt = temp;
		...
		pNetServer->GetMonitoringInfo(AcceptTps, RecvTpsSum, SendTpsSum,
...
unsigned __stdcall CNetServer::netAcceptThread(void* pParameter)
{
	CNetServer* pNetServer = (CNetServer*)pParameter;
	srand((unsigned int)GetCurrentThreadId());
	...
	while (true)
	{
		...
		SOCKET clientSocket = accept(pNetServer->m_ListenSocket, (sockaddr*)&clientaddr, &clientLen);
		...
		// 세션 생성
		st_SESSION* pSession = pNetServer->CreateSession(clientSocket);
		if (pSession == nullptr)
		{
		...
		if (pNetServer->OnConnectionRequest(pSession->IpStr, pSession->usPort) == false)
		{
			LOG(L"Error", en_LOG_LEVEL::LEVEL_ERROR, L"[ IP %s, Port %d ] Banned ", pSession->IpStr, pSession->usPort);
			pNetServer->DecreaseIOCount(pSession);
			continue;
		}

		// AcceptTps
		pNetServer->m_acceptCount++;

		// 송신버퍼 0으로 해서 비동기 방식으로 작동
		if (pNetServer->m_bOverlappedSend == true)
		{
			int sendSize = 0;
			setsockopt(clientSocket, SOL_SOCKET, SO_SNDBUF, (char*)&sendSize, sizeof(int));
		}

		CreateIoCompletionPort((HANDLE)clientSocket, pNetServer->m_hIOCP, (ULONG_PTR)pSession, 0);

		DEBUG(DEBUG_LOCATION::ACCEPT_BEF_ONCLIENTJOIN, pSession);

		// 콘텐츠 작업 파트
		pNetServer->OnClientJoin(pSession->SessionID);

		DEBUG(DEBUG_LOCATION::ACCEPT_AFT_ONCLIENTJOIN, pSession);

		DEBUG(DEBUG_LOCATION::ACCEPT_BEF_RECVPOST, pSession);

		int retRecvPost = pNetServer->netWSARecvPost(pSession);
		if (retRecvPost != RET_SUCCESS)
		{
			pNetServer->DisconnectSession(pSession);
		}
...
unsigned __stdcall CNetServer::netIOThread(void* pParameter)
{
	CNetServer* pNetServer = (CNetServer*)pParameter;
	srand((unsigned int)GetCurrentThreadId());
	...
	while (1)
	{
		...
		GetQueuedCompletionStatus(pNetServer->m_hIOCP, &transferred, (PULONG_PTR)&pSession, (LPOVERLAPPED*)&pOverlapped, INFINITE);
...
#ifdef MY_DEBUG
		pSession->tlsIndex = pNetServer->m_tlsDebugIndex;
		...
		else
		{
			if (pOverlapped->recvFlags == true)
			{
				...
				else
				{
					...
					int retRecv = pNetServer->netWSARecv(pSession);
			...
			{
				// SendTps
				long long* pSendTps = (long long*)TlsGetValue(pNetServer->m_tlsSendTpsIndex);
				if (pSendTps == nullptr)
				{
					long long index = (long long)TlsGetValue(pNetServer->m_tlsIndex);
					if (index == 0)
					{
						index = InterlockedIncrement((LONG*)&pNetServer->m_tlsCount);
						TlsSetValue(pNetServer->m_tlsIndex, (LPVOID)index);
					}
					pSendTps = &pNetServer->m_pTlsSendTpsArr[index];
					TlsSetValue(pNetServer->m_tlsSendTpsIndex, pSendTps);
				}
				...
				int retSendPost = pNetServer->netWSASendPost(pSession);
...
int CNetServer::netWSARecv(st_SESSION* pSession)
{
	// RecvTps
	...
	while (1)
	{
		...
		if (RecvPacket->Decryption(Header) == false)
		{
			...
			DEBUG(DEBUG_LOCATION::RECV_DECRYPTION_FAIL, pSession);
...
int CNetServer::netWSARecvPost(st_SESSION* pSession)
{
	int afterEnqSize = 0;
...
int CNetServer::netWSASendPacket(unsigned long long SessionID, CSerializeBuffer* pPacket)
{
...
int CNetServer::netWSASendEnq(unsigned long long SessionID, CSerializeBuffer* pPacket)
{
	st_SESSION* pSession = AcquireLock(SessionID);
	if (pSession == nullptr)
	{
...
int CNetServer::netWSASendPost(st_SESSION* pSession)
{
...
st_SESSION* CNetServer::CreateSession(SOCKET clientSocket)
{
	int idx = 0;
...
bool CNetServer::GetIP_Port(unsigned long long SessionID, wchar_t* outIpStr, unsigned short* outPortNum)
{
	st_SESSION* pSession = AcquireLock(SessionID);
	if (pSession == nullptr)
		return false;
...
// 세션 연결 종료 처리
void CNetServer::DisconnectSession(unsigned long long SessionID)
{
	st_SESSION* pSession = AcquireLock(SessionID);
	if(pSession == nullptr)
		return;
...
void CNetServer::DecreaseIOCount(st_SESSION* pSession)
{
#ifdef DEBUG
...
void CNetServer::PrintError(st_SESSION* pSession, const wchar_t* errMsg, int errorCode)
{	 
...
st_SESSION* CNetServer::AcquireLock(unsigned long long SessionID)
{
...
void CNetServer::ReleaseLock(st_SESSION* pSession)
{
